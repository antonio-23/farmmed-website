...
export const all_prescription = (req, res) => {
    const q = `SELECT r.key, DATE_FORMAT(r.Date, "%Y-%m-%d") AS data_wystawienia, CONCAT(u.first_name, " ", u.last_name) AS user_name, (SELECT CONCAT(u1.first_name, " ", u1.last_name) FROM farmmed.user u1 INNER JOIN farmmed.recepty r1 ON u1.id = r1.id_doctor WHERE r1.id = r.id) AS doctor_name, DATE_FORMAT(r.validity_date, "%Y-%m-%d") AS ważnosc_recepty, d.Nazwa_Produktu_Leczniczego AS nazwa_leku, d.moc AS moc, rl.opakowanie AS opakowanie, rl.dawkowanie AS dawkowanie FROM farmmed.recepty r INNER JOIN farmmed.user u ON r.id_user = u.id INNER JOIN farmmed.recepta_leki rl ON r.id = rl.id_recepty INNER JOIN farmmed.drugs d ON rl.id_leku = d.Identyfikator_Produktu_Leczniczego WHERE u.id = ? ORDER BY r.id DESC`;
    db.query(q, [req.body.user], (err, data) =>{
        if(err) return res.status(500).send(err);
        else return res.status(200).send(data);
    })
